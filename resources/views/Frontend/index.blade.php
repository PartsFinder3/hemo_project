...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateFindButton() {
        const hasBasicInfo = makeSelect.value && modelSelect.value && yearSelect.value;
        const hasParts = selectedParts.length > 0;
        findBtn.disabled = !(hasBasicInfo && hasParts);
    }

    // Create/remove part tags
    function createPartTag(partId) {
        const partOption = document.querySelector(`#parts-dropdown option[value="${partId}"]`);
        const partName = partOption ? partOption.textContent : partId;
        const tag = document.createElement("div");
        tag.className = "part-tag";
        tag.innerHTML = `${partName}<input type="hidden" name="parts[]" value="${partId}"><button type="button" class="remove-btn">&times;</button>`;
        tag.querySelector(".remove-btn").addEventListener("click", () => removePart(partId, tag));
    ...
    makeSelect.addEventListener("change", async function () {
        ...
        modelChoices.setChoices([{ value: '', label: 'Loading...', disabled: true, selected: true }], 'value', 'label', true);
        ...
        if (!makeId) {
            modelChoices.clearStore();
            modelChoices.setChoices([{ value: '', label: 'Select Your Model', selected: true }], 'value', 'label', true);
            updateFindButton();
            return;
        }
        ...
        try {
            ...
            if (Array.isArray(data) && data.length > 0) {
                const choices = [{ value: '', label: 'Select Your Model', selected: true }, ...data.map(m => ({ value: String(m.id), label: m.name }))];
                modelChoices.setChoices(choices, 'value', 'label', true);
                showFormGroup(modelGroup);
                modelChoices.enable();
            } else {
                modelChoices.setChoices([{ value: '', label: 'No models available', disabled: true, selected: true }], 'value', 'label', true);
                showFormGroup(modelGroup);
            }
        } catch (err) {
            console.error(err);
            modelChoices.clearStore();
            modelChoices.setChoices([{ value: '', label: 'Error loading models', disabled: true, selected: true }], 'value', 'label', true);
            showFormGroup(modelGroup);
        }
        updateFindButton();
    });
    ...
    findBtn.addEventListener("click", function (e) {
        if (findBtn.disabled) { e.preventDefault(); return; }
        const conditionElement = document.querySelector('input[name="condition"]:checked');
        if (!conditionElement) { e.preventDefault(); alert("Please select a condition option"); return; }
        const searchData = {
    ...
    function setupPagination(gridId, paginationId, perPage = 6) {
        ...
        function showPage(page) {
            products.forEach((product, i) => {
                product.style.display = i >= (page - 1) * perPage && i < page * perPage ? "block" : "none";
            });
            pagination.querySelectorAll("button").forEach((btn, i) => btn.classList.toggle("active", i + 1 === page));
        }
    ...
    // Scroll to ads section if requested
    if (document.location.hash === "#ads") {
        const element = document.getElementsByClassName("scroll_posint")[0];
...
function callSupplier(isActive, number) {
    if (isActive == 1) window.location.href = `tel:${number}`;
...
function contactSupplier(isActive, whatsapp, title) {
    if (isActive === '1') {
        ...
        const cleanWhatsapp = whatsapp.replace(/\D/g, '');
        window.open(`https://wa.me/${cleanWhatsapp}?text=${message}`, '_blank');
    } else alert('Supplier is currently inactive');
}
