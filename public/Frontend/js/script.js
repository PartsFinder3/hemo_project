...
function createPartTag(partId, partName) {
    const tag = document.createElement("div");
    tag.className = "part-tag";
    tag.innerHTML = `
        ${partName}
...
function removePart(partId, tagElement) {
    selectedParts = selectedParts.filter((p) => p !== partId);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    toastElList.map(function (toastEl) {
        const toast = new bootstrap.Toast(toastEl, { delay: 4000 }); // 4 sec auto hide
        toast.show();
    });
});
